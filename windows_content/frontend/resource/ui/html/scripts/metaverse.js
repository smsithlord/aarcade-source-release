function Metaverse(eventHandler)
{
	...
	this.defaultInfo = {
		"created":
		{
			"default": 0,//firebase.database.ServerValue.TIMESTAMP
			"types": "timestamp",
			"format": "/^[-+]?\\d+$/i",
	...
	this.defaultInstance = {
		...
		"avatar":
		{
			...
			"types": "uri",
			"format": "/((((http|https):\\/\\/|(www\\.|www\\d\\.))([^\\-][a-zA-Z0-9\\-]+)?(\\.\\w+)(\\/\\w+){0,}(\\.\\w+){0,}(\\?\\w+\\=\\w+){0,}(\\&\\w+\\=\\w+)?)|(^$))/i",
			"formatDescription": "An avatar URL to represent this instance's author."
		},
		"author":
		{
			"label": "Author",
			...
			/*"format": "/^.*$/i",*/
			/*"format": "/^.{0,1024}$/i",*/
			"formatDescription": "The display name of the author of this instance."
		},
		"title":
		{
			"label": "Title",
	...
	this.defaultObject = {
		...
		"child":
		{
			...
			"types": "integer",
			"format": "/(0|1)$/i",
			"formatDescription": "Child must be 0 or 1."
		},
		"parentObject":
		{
			"label": "Parent Object",
		...
		"anim":
		{
			...
			"types": "string",
			"format": "/^.*$/i",
			"formatDescription": "Sequence name.  "
		}
	};
...
var g_firebaseRootRef;
function createFirebaseGlobal()
{
	if( !!!g_firebaseRootRef )
	{
		var config = {
			//apiKey: "<API_KEY>",
			//authDomain: "<PROJECT_ID>.firebaseapp.com",
			databaseURL: "https://metaverse.firebaseio.com/",//this.quickJoinAddress,
			projectId: "metaverse",
			storageBucket: "gs://firebase-metaverse.appspot.com"//,	// TODO: make this a variable too, just like the quickJoinAddress.
			//messagingSenderId: "<SENDER_ID>",
		};
		firebase.initializeApp(config);
		g_firebaseRootRef = firebase.database().ref();
	}

	return g_firebaseRootRef;
};

Metaverse.prototype.connect = function(server, callback, firebaseRootOverride)
{
	if( this.sessionEnded )
		return;
	...
	if( this.root !== "local" )
	{
		if( !!!firebaseRootOverride)
			this.rootRef = createFirebaseGlobal();//new Firebase(this.root);
		else
			this.rootRef = firebaseRootOverride;
...
Metaverse.prototype.createUniverse = function(title, key, userBans, isPublic, isPersistent, callback)
{
	if( this.sessionEnded )
		return;
...
Metaverse.prototype.fetchAllMapIds = function(universeId, callback)
{
	...
	infoRequest.open("GET", this.quickJoinAddress + universeId + "/library/maps/.json?shallow=true", true);
...
Metaverse.prototype.fetchAllModelIds = function(universeId, callback)
{
	...
	infoRequest.open("GET", this.quickJoinAddress + universeId + "/library/models/.json?shallow=true", true);
	infoRequest.send(null);
};
...
Metaverse.prototype.fetchAllUniverseIds = function(callback)
{
	...
	infoRequest.open("GET", this.quickJoinAddress + ".json?shallow=true", true);
	infoRequest.send(null);
};
...
/*
	...
	infoRequest.open("GET", this.quickJoinAddress + this.universe + "/library/instances/.json?shallow=true", true);
	infoRequest.send(null);
};
/*
...
Metaverse.prototype.userSessionUpdated = function(userId, snapshot)
{
	...
	{
		// if this is where a server shutting down is detected, then shouldn't this make sure thats the case?
		//if( sessionId !== "" )
		//{
	...
	else if( !!session )
	{
		...
		var testVal = (!!session.state) ? session.state.id : this.defaultSession.state.id;
		if( testVal !== "" && (!!!oldSession || !!!oldSession.state || oldSession.state.id != testVal) )
			fields.state = {"id": testVal};

		var testVal = (!!session.launched) ? session.launched.id : this.defaultSession.launched.id;
		if( testVal !== "" && (!!!oldSession || !!!oldSession.launched || oldSession.launched.id != testVal) )
			fields.launched = {"id": testVal};

		var testTwitchChannelVal = (!!session.twitch) ? session.twitch.channel : this.defaultSession.twitch.channel;
		var testTwitchLiveVal = (!!session.twitch) ? session.twitch.live : this.defaultSession.twitch.live;
		if( !!!oldSession || !!!oldSession.twitch || oldSession.twitch.channel != testTwitchChannelVal || oldSession.twitch.live != testTwitchLiveVal )
			fields.twitch = {"channel": testTwitchChannelVal, "live": testTwitchLiveVal};
...
Metaverse.prototype.mapChanged = function(child, prevChildKey)
{
	...
	var val = child.val();
	if( !!!val || !!!val.info || !!!val.info.id )
	{
		/*
		this.sessionEnded = true;
		this.reset();
		if( typeof window.onSessionEnded === "function" )
			window.onSessionEnded();*/
		return;
	}
				...
				else
				{
					...
					this.universeRef.child("info").child("owner").once("value", function(ownerSnapshot)
					{
						...
						{	
							...
							this.connectedRef = firebase.database().ref(".info/connected");//new Firebase(this.root + ".info/connected");
							...
							function connectedRefUpdate(connectedSnapshot)
							{
								...
								if( connectedSnapshot.val() === true )
								{
									if( this.localUserRef )
									{
										...
										this.localUserRef.onDisconnect().update({"lastLogOut": firebase.database.ServerValue.TIMESTAMP});

										var sessionKey = this.sessionRef.key;
										var updateData = {};
										//updateData["sessions/" + sessionKey + "/status"] = "Online";
										updateData["sessions/" + sessionKey + "/created"] = firebase.database.ServerValue.TIMESTAMP;
										updateData["lastLogIn"] = firebase.database.ServerValue.TIMESTAMP;
										...
										/*
										this.sessionRef.set({"presence": {}, "status": "Online", "timestamp": firebase.database.ServerValue.TIMESTAMP}, function(error)
										{
...
Metaverse.prototype.createUser = function(data, callback, userIdOverride)
{
	...
	data.lastLogOut = 0;
	data.lastLogIn = firebase.database.ServerValue.TIMESTAMP;
	ref.set(data);
...
Metaverse.prototype.sendInstanceObjectRemove = function(object, instance, item, model, slave, child, parentObject, scale, origin, angles, callback)
{
	...
	this.instanceObjectsRef.child(object).set(null, function()
	{
		...
		updateData["current/info/removed"] = firebase.database.ServerValue.TIMESTAMP;
		updateData[this.localUser.id + "/info/removed"] = firebase.database.ServerValue.TIMESTAMP;
...
Metaverse.prototype.sendInstanceUserUpdate = function(instance, say, bodyOrigin, bodyAngles, headOrigin, headAngles, item, object, mouseX, mouseY, webURL, avatarURL, displayName, state, launched, twitchChannel, twitchLive)
{
	if( this.sessionEnded )
		return;
	...
	if( (!!say && !this.localUser.lastUpdate.hasOwnProperty("say")) || (!!say && say != this.localUser.lastUpdate.say) || (!!!say && this.localUser.lastUpdate.say !== "") )
	{
		...
		updates['/say/modified'] = firebase.database.ServerValue.TIMESTAMP;
	...
	if( (!!bodyOrigin && !this.localUser.lastUpdate.hasOwnProperty("bodyOrigin")) || (!!bodyOrigin && bodyOrigin != this.localUser.lastUpdate.bodyOrigin) || (!!!bodyOrigin && this.localUser.lastUpdate.bodyorigin !== "") )
	{
		...
		updates['/transform/body/modified'] = firebase.database.ServerValue.TIMESTAMP;
	...
	if( (!!headOrigin && !this.localUser.lastUpdate.hasOwnProperty("headOrigin")) || (!!headOrigin && headOrigin != this.localUser.lastUpdate.headOrigin) || (!!!headOrigin && this.localUser.lastUpdate.headOrigin !== "") )
	{
		...
		updates['/transform/head/modified'] = firebase.database.ServerValue.TIMESTAMP;
	...
	if( (!!item && !this.localUser.lastUpdate.hasOwnProperty("item")) || (!!item && item != this.localUser.lastUpdate.item) || (!!!item && this.localUser.lastUpdate.item !== "") )
	{
		...
		updates['/item/modified'] = firebase.database.ServerValue.TIMESTAMP;
	...
	if( (!!object && !this.localUser.lastUpdate.hasOwnProperty("object")) || (!!object && object != this.localUser.lastUpdate.object) || (!!!object && this.localUser.lastUpdate.object !== "") )
	{
		...
		updates['/object/modified'] = firebase.database.ServerValue.TIMESTAMP;
	...
	if( (!!mouseX && !this.localUser.lastUpdate.hasOwnProperty("mouseX")) || (!!mouseX && mouseX != this.localUser.lastUpdate.mouseX) || (!!!mouseX && this.localUser.lastUpdate.mouseX !== parseFloat("0")) )
	{
		...
		updates['/mouse/modified'] = firebase.database.ServerValue.TIMESTAMP;
	...
	if( (!!webURL && !this.localUser.lastUpdate.hasOwnProperty("webURL")) || (!!webURL && webURL != this.localUser.lastUpdate.webURL) || (!!!webURL && this.localUser.lastUpdate.webURL !== "") )
	{
		...
		updates['/web/modified'] = firebase.database.ServerValue.TIMESTAMP;
	...
	if( (!!avatarURL && !this.localUser.lastUpdate.hasOwnProperty("avatarURL")) || (!!avatarURL && avatarURL != this.localUser.lastUpdate.avatarURL) || (!!!avatarURL && this.localUser.lastUpdate.avatarURL !== "") )
	{
		...
		updates['/avatar/modified'] = firebase.database.ServerValue.TIMESTAMP;
		updates['/avatar/url'] = avatarURL;
		this.localUser.lastUpdate.avatarURL = avatarURL;
	}
	
	if( (!!state && !this.localUser.lastUpdate.hasOwnProperty("state")) || (!!state && state != this.localUser.lastUpdate.state) || (!!!state && this.localUser.lastUpdate.state !== "") )
	{
		if( !!!state )
			state = 0;

		updates['/state/modified'] = firebase.database.ServerValue.TIMESTAMP;
		updates['/state/id'] = state;
		this.localUser.lastUpdate.state = state;
	}
	
	if( (!!launched && !this.localUser.lastUpdate.hasOwnProperty("launched")) || (!!launched && launched != this.localUser.lastUpdate.launched) || (!!!launched && this.localUser.lastUpdate.launched !== "") )
	{
		if( !!!launched )
			launched = "";

		updates['/launched/modified'] = firebase.database.ServerValue.TIMESTAMP;
		updates['/launched/id'] = launched;
		this.localUser.lastUpdate.launched = launched;
	}
	
	if( (!!twitchChannel && !this.localUser.lastUpdate.hasOwnProperty("twitchChannel")) || (!!twitchChannel && twitchChannel != this.localUser.lastUpdate.twitchChannel) || (!!!twitchChannel && this.localUser.lastUpdate.twitchChannel !== "") )
	{
		if( !!!twitchChannel )
			twitchChannel = "";

		updates['/twitch/modified'] = firebase.database.ServerValue.TIMESTAMP;
		updates['/twitch/channel'] = twitchChannel;
		this.localUser.lastUpdate.twitchChannel = twitchChannel;
	}
	
	if( (!!twitchLive && !this.localUser.lastUpdate.hasOwnProperty("twitchLive")) || (!!twitchLive && twitchLive != this.localUser.lastUpdate.twitchLive) || (!!!twitchLive && this.localUser.lastUpdate.twitchLive !== "") )
	{
		if( !!!twitchLive )
			twitchLive = 0;

		updates['/twitch/modified'] = firebase.database.ServerValue.TIMESTAMP;
		updates['/twitch/live'] = twitchLive;
		this.localUser.lastUpdate.twitchLive = twitchLive;
	}
...
Metaverse.prototype.connectInstance = function(instanceId, callback)
{
	...
	this.instanceRef.child("current").on("value", function(snapshot)
	{
		...
		{
			//console.log(JSON.stringify(this.instanceInfo));
			this.sessionEnded = true;
			this.reset();
			if( typeof window.onSessionEnded === "function" )
				window.onSessionEnded();
		...
		if( !!!this.library.instances[instanceId] || window.isQuickHosting === true )
		{
			...
			if( !!this.instanceInfo && !!this.instanceInfo.map )
			{
				...
				{
					if( !!this.instanceInfo && !!!this.library.maps[this.instanceInfo.info.map] )
					{
...
*/
			var timestamp = 0; //firebase.database.ServerValue.TIMESTAMP
			var updateData = {};
			...
			this.sessionRef.update(updateData, function()
			{
				if( !this.isConnectedToInstance )
				{
					...
					this.connectInstance.call(this, instanceId, function()
					{
						...
						//aaapi.cmd("networkEvent", "hostReady", quickConnectInfo.address, quickConnectInfo.universe, quickConnectInfo.instance, metaverse.sessionRef.key);
					});
				}
				else
					callback(key);
